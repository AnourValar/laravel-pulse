...
<x-pulse::card :cols="$cols" :rows="$rows" :class="$class">
    ...
    <style>
    .after\:border-l-red-500:after{content:var(--tw-content);--tw-border-opacity: 1;border-left-color:rgb(228 59 96 / var(--tw-border-opacity))}
    html.dark #anourvalar-http-requests-count-svg {fill: #9ca3af !important;}
    html:not(.dark) #anourvalar-http-requests-count-svg {fill: #4b5563 !important;}
    </style>
</x-pulse::card>
...
Alpine.data('requestChart', (config) => ({
    init() {
        ...
        Livewire.on('requests-count-chart-update', ({ requests }) => {
            if (chart === undefined) {
                return
            }
